...
def generate_user_data(n_users=500, fake_ratio=0.2): #fake/real users
    data = []
    for i in range(n_users):
        is_fake = int(i < int(n_users * fake_ratio)) 
        user_id = f"user_{i+1}"
        creation_date = fake.date_between(start_date='-1y', end_date='-1d')
        num_sessions = random.randint(3, 15) if not is_fake else random.randint(20, 50) #Random number of sessions
        
        for _ in range(num_sessions):
            login_time = fake.date_time_between(start_date=creation_date, end_date='now')
            session_duration = random.uniform(300, 3600) if not is_fake else random.uniform(30, 300)   #session durations
